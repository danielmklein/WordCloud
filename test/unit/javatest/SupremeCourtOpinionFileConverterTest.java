...
public class SupremeCourtOpinionFileConverterTest
{
    ...
    private void createTestFile(List<String> fileLines) throws IOException
    {
        ...
        } catch (IOException e)
        {
            throw new IOException(
                            "Something went wrong while creating the test file "
                                            + TEST_FILE_PATH);
        } finally
        {
            if (pw != null)
    ...
    public void setUp() throws Exception
    {
        ...
        try
        {
            TEST_FILE_PATH = (new File(".")).getCanonicalPath()
                            + File.separator
                            + "MURPHY_1944 U.S. LEXIS 1230.txt";
            TEST_SERIALIZED_PATH = (new File(".")).getCanonicalPath()
                            + File.separator + "serialized_test_doc";
        } catch (IOException e)
        {
            throw new IOException(
                            "Something went wrong while building the TEST_FILE_PATH or TEST_SERIALIZED_PATH.");
        }
    ...
    public void tearDown() throws Exception
    {
        ...
        if (testFile.isFile())
        {
            ...
            {
                throw new Exception(
                                "Something went wrong while deleting the file at "
                                                + TEST_FILE_PATH);
            }
        }
    }
    ...
    public void testNormalCase() throws IOException
    {
        ...
        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_CASE_TITLE), CASE_TITLE);

        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_OPIN_AUTHOR), OPINION_AUTHOR);

        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_OPIN_TYPE), OPINION_TYPE);

        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_CASE_NUM), CASE_NUM);

        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_US_CITE), CASE_US_CITE);
        ...
        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_LEXIS_CITE), CASE_LEXIS_CITE);

        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_FULL_CITE), CASE_FULL_CITE);

        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_CASE_DATES), CASE_DATES);

        assertEquals(convertedOpinion.getMetadata().getField(
                        WordCloudConstants.META_DISPOSITION), CASE_DISPOSITION);
    ...
    public void testNoNoMetadataInFile() throws IOException
    {
        ...
        this.createTestFile(VALID_OPINION_LINES.subList(10,
                        VALID_OPINION_LINES.size()));
        SupremeCourtOpinion convertedOpinion = this.testConverter.convertFile();
