...
class WordCloudMain():
    ...
    def sort_opinions(self, opinion_list):
        ...
        scalia_subset = sorter.create_subset(sort_field, ["Scalia", "SCALIA"])
        
        sorter = DocumentSorter(scalia_subset)
        scalia_free_speech = sorter.create_subset("case_lexis_cite", 
                                                  ACCEPTED_CITES)
        subsets = [scalia_free_speech]
        ###
    ...
    def run_analysis(self, subsets):
        ...
        subset_lists = analysis_engine.analyze_docs(500)
        return subset_lists
    ...
    def generate_clouds(self, subset_lists):
        ...
        '''
        scalia_file = subset_lists[0][0] + "_scalia_free_speech.jpg"
        scalia_terms = subset_lists[0][1]
        # test output
        print "SCALIA TERMS: {0}".format(scalia_terms)
        print "SCALIA OUTPUT FILE: {0}".format(scalia_file)
        # /test output
        
        print "generating a word cloud for each subset..."
        scalia_cloud_gen = WordCloudGenerator(scalia_terms, scalia_file)
        scalia_cloud_gen.generate_word_cloud()
